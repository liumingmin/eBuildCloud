...
(function (wid, dcm) {
	...
	function generateItemBodyDom(year, month, cal) {
		...
		loop(0, dateArr.length, function (i) {
			if (/b$/.test(dateArr[i])) {
				...
				var bonusText ="&nbsp;";
				if (tempStamp<=cal.recentStamp){
					if (cal.bonusAddVisible){
						bonusText = "+";
					}
					if (cal.bonus){
						var bonus = cal.bonus[tempStamp];
						if(bonus && bonus>0){
	             bonusText = (""+bonus);
						}
					}
				...
				html += '<li' + (tempStamp >= cal.beginStamp && tempStamp <= cal.endStamp ?
					...
					'><i data-stamp="' + tempStamp + '">' + dateArr[i] + '</i> <div class="aui-text-primary" data-stamp="'+tempStamp+'" style="margin-top:-10px">'+bonusText+'</div></li>';
			}
		});
		return html + '</ul>';
	}
	...
	function switchItemBody(direct, distance, cal) {
		...
		var objs = cal.box.querySelectorAll('.calendar-item.calendar-item' + itemNum);
		$(objs).each(function () {
			obj = this;
		...
		cal.switchRender(cal.currentYear, cal.currentMonth, cal);
	}

	function refreshUI(cal){
		var objs = cal.box.querySelectorAll('.calendar-item.calendar-item'+cal.currentIdx);
		$(objs).each(function () {
			obj = this;
			obj.innerHTML = generateItemBodyDom(cal.currentYear, cal.currentMonth, cal);
			obj.setAttribute('data-year', cal.currentYear);
			obj.setAttribute('data-month', cal.currentMonth + 1);
		});
		//cal.switchRender(cal.currentYear, cal.currentMonth, cal);

		cal.renderCallbackArr(cal.beforeRenderArr);
	}
	...
	function touchEnd(event, cal) {
		...
		if (cal.end.time - cal.start.time < 200 && Math.abs(tempDis) < 10) {
			if ($(event.target).is('li') && event.target.className !== 'calendar-disabled'
			|| $(event.target).is('i') && event.target.parentNode.className !== 'calendar-disabled'
		 ||  $(event.target).is('div') && event.target.parentNode.className !== 'calendar-disabled') {
	...
	function Calendar(config) {
		...
		this.bonusAddVisible = config.bonusAddVisible || false;
